...
class TTS:
    ...
    def _get_ref_spec(self, ref_audio_path):
        # --- 修改开始：放弃 torchaudio.load，改用 librosa ---
        import librosa
        import numpy as np
        
        try:
            # 使用 librosa 加载音频 (纯 CPU 处理，不涉及 CUDA 驱动)
            # sr=None 表示保持原始采样率
            audio_np, raw_sr = librosa.load(ref_audio_path, sr=None)
            
            # 将 numpy 转换为 torch tensor 并转到对应的设备
            raw_audio = torch.from_numpy(audio_np)
            
            # 如果是单声道，增加一个维度变成 [1, T]；如果是多声道，取均值
            if len(raw_audio.shape) == 1:
                raw_audio = raw_audio.unsqueeze(0)
            elif raw_audio.shape[0] > 1:
                raw_audio = raw_audio.mean(0, keepdim=True)
                
        except Exception as e:
            print(f"音频加载失败: {ref_audio_path}, 错误: {str(e)}")
            raise e
        # --- 修改结束 ---
